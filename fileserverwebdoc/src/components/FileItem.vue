        ...
        <a-col :xs="3" :sm="1" style="text-align: center">
          <a-popover v-model="popover" placement="topRight">
            <template slot="content">
              <a-button-group>
                <a-button v-if="type=='file'" icon="login" size="small" @click="openfile">{{breakpoint?'':'Open'}}</a-button>
                <a-button icon="rollback" size="small" @click="removefile">{{breakpoint?'':'Move'}}</a-button>
                <a-button icon="edit" size="small" @click="renamefile">{{breakpoint?'':'Rename'}}</a-button>
                <a-button v-if="type=='file'" icon="cloud-download" size="small" @click="downloadfile">{{breakpoint?'':'Download'}}</a-button>
                <a-button icon="delete" size="small" @click="ddelfile">{{breakpoint?'':'Delete'}}</a-button>
              </a-button-group>
            </template>
...
export default {
  ...
  computed: {
    getfilesize: function() {
      ...
      else fileSizeMsg = "File Size >1TB";
      return fileSizeMsg;
    },
    gettime: function() {
